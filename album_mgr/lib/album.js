...
Album.prototype.__photos = null;

Album.prototype.photos = function (callback) {
	if(this.__photos != null) {
		callback(null, this.__photos);
		return;
	}
	...
	fs.readdir(self.path, function (err, files) {
		...
		(function iterator(index) {
			if(index == files.length) {
				self.__photos = only_files;
				callback(null, self.__photos);
				return;
			}
